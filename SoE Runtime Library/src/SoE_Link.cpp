...
BOOL FAR PASCAL EnumSessionsCallback(LPCDPSESSIONDESC2 lpSessionDesc,
									 ...
									 LPVOID lpContext)
{
	SoELinkSession* Session = (SoELinkSession*) lpContext;
...
{
	SoELinkPlayer* Player = (SoELinkPlayer*) lpContext;
...
BOOL SoELink::CreateIPX(void* hWnd)
{
...
BOOL SoELink::CreateTCPIP(void* hWnd, char* IPAddress)
{
	// Create the DirectPlay object
	rval = CoCreateInstance(CLSID_DirectPlay, NULL, CLSCTX_INPROC_SERVER,
		IID_IDirectPlay3A, (LPVOID*)&m_DP);
...
BOOL SoELink::CreateModem(void* hWnd, char* PhoneNo)
{
	// Create the DirectPlay object
	rval = CoCreateInstance(CLSID_DirectPlay, NULL, CLSCTX_INPROC_SERVER,
		IID_IDirectPlay3A, (LPVOID*)&m_DP);
...
BOOL SoELink::CreateSerial(void* hWnd)
{
	// Create the DirectPlay object
	rval = CoCreateInstance(CLSID_DirectPlay, NULL, CLSCTX_INPROC_SERVER,
		IID_IDirectPlay3A, (LPVOID*)&m_DP);
...
BOOL SoELink::EnumSessions(SoELinkSession* Sessions, GUID APP_GUID)
{
...
BOOL SoELink::HostSession(SoELinkSession* Session)
{
	DPSESSIONDESC2 SessionDesc;
...
BOOL SoELink::JoinSession(SoELinkSession* Session)
{
	DPSESSIONDESC2 SessionDesc;
...
BOOL SoELink::CloseSession(void)
{
	rval = m_DP->Close();
	if (rval != DP_OK)
		return FALSE;
...
BOOL SoELink::EnumPlayers(SoELinkPlayer* Players, DWORD Flags)
{
...
BOOL SoELink::CreatePlayer(SoELinkPlayer* Player)
{
	DPNAME Name;
...
BOOL SoELink::DestroyPlayer(SoELinkPlayer* Player)
{
...
HRESULT SoELink::Send(DPID idFrom, DPID idTo, DWORD dwFlags,
											LPVOID lpData, DWORD dwDataSize)
{
	return m_DP->Send(idFrom, idTo, dwFlags, lpData, dwDataSize);
}

HRESULT SoELink::Receive(LPDPID lpidFrom, LPDPID lpidTo,	DWORD dwFlags,
