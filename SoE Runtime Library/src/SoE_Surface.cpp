...
void SoESurface::InitSoESurface(void)
{
    ...
    m_MemoryLocation     = SOEMEM_SYSTEMONLY;
    m_SurfaceId          = -1;
...
void SoESurface::FunctionMapper(void)
{
    ...
    {
		case 8:
			m_fpPutPixel = &SoESurface::PutPixel8;
			m_fpPutAAPixel = &SoESurface::PutAAPixel8;
			m_fpGetPixel   = &SoESurface::GetPixel8;
			m_fpVLine      = &SoESurface::VLine8;
			m_fpHLine      = &SoESurface::HLine8;
			break;
			
		case 15:
			m_fpPutPixel   = &SoESurface::PutPixel16;
			m_fpPutAAPixel = &SoESurface::PutAAPixel15;
			m_fpGetPixel   = &SoESurface::GetPixel16;
			m_fpVLine      = &SoESurface::VLine16;
			m_fpHLine      = &SoESurface::HLine16;
			break;
			
		case 16:
			m_fpPutPixel   = &SoESurface::PutPixel16;
			m_fpPutAAPixel = &SoESurface::PutAAPixel16;
			m_fpGetPixel   = &SoESurface::GetPixel16;
			m_fpVLine      = &SoESurface::VLine16;
			m_fpHLine      = &SoESurface::HLine16;
			break;
		...
		case 24:
			m_fpPutPixel   = &SoESurface::PutPixel24;
			m_fpPutAAPixel = &SoESurface::PutAAPixel24;
			m_fpGetPixel   = &SoESurface::GetPixel24;
			m_fpVLine      = &SoESurface::VLine24;
			m_fpHLine      = &SoESurface::HLine24;
			break;
			
		case 32:
			m_fpPutPixel   = &SoESurface::PutPixel32;
			m_fpPutAAPixel = &SoESurface::PutAAPixel32;
			m_fpGetPixel   = &SoESurface::GetPixel32;
			m_fpVLine      = &SoESurface::VLine32;
			m_fpHLine      = &SoESurface::HLine32;
			break;
...
SoESurface::SoESurface(SoESurface& aSurface)					
{
	HRESULT rval;
    RECT srcRect;
...
SoESurface::~SoESurface()
{
	SOELOG("SoESurface::~SoESurface");
	...
	GetScreen()->ReleaseSurfaceFromList(this);
}

HRESULT SoESurface::Create(SoEScreen* pScreen, int Width, int Height, BYTE memoryType)
{
	HRESULT rval;
...
HRESULT SoESurface::Create(SoEScreen* pScreen, const char* szFilename, BYTE memoryType)
{
    SoEImage* image;
    HRSRC hResInfo;
	HGLOBAL hResData;
    ...
    // Create the SoEImage object
    image = new SoEImage();
	
    // Load the file to a DX surface
    m_lpDDS = image->GetImage(pScreen->GetDD(), szFilename, memoryType, SOEIMAGE_UNKNOWN);
	
    // Delete the SoEImage object
    delete image;
	...
	if (m_lpDDS == NULL) 
    {
		...
		if (((hResInfo  = FindResource(NULL, szFilename, RT_BITMAP)) == NULL))
			return -2;
		...
		// m_lpDDS = image->GetImage(pScreen->GetDD(),0 ,(CHAR*)pvRes, memoryType, SOEIMAGE_UNKNOWN);
...
HRESULT SoESurface::Create(SoEScreen* pScreen, LONG lSize, fstream* fs, BYTE memoryType)
{
    SoEImage* image;
    ...
    // Load the file to a DX surface
    m_lpDDS = image->GetImage(pScreen->GetDD(), lSize, fs, memoryType, SOEIMAGE_UNKNOWN);
...
HRESULT SoESurface::Create(SoEScreen* pScreen, LONG lSize, CHAR* lpCache, BYTE memoryType)
{
    SoEImage* image;
    CHAR* cache;
    ...
    // Load the file to a DX surface
    m_lpDDS = image->GetImage(pScreen->GetDD(), lSize, cache, memoryType, SOEIMAGE_UNKNOWN);
...
HRESULT SoESurface::Create(SoEScreen* pScreen, LONG lSize, FILE* fh, BYTE memoryType)
{
    SoEImage* image;
    ...
    // Load the file to a DX surface
    m_lpDDS = image->GetImage(pScreen->GetDD(), lSize, fh, memoryType, SOEIMAGE_UNKNOWN);
...
BOOL SoESurface::LoadAlphaMask(const char* szFilename)
{
	FILE* fhMask;
	LONG width, height;
	DWORD byteCount;
...
HRESULT SoESurface::SetAlphaMaskPointer(BYTE* lpAlpha)
{
    if (m_AlphaMask != NULL)
        delete[] m_AlphaMask;
...
HRESULT SoESurface::InvertColors(int x1, int y1, int x2, int y2)
{
    int x, y, tmp;
    int bpp;
    int pitch;
	...
	if (x2 >= m_ClipRect.right)
		x2 = m_ClipRect.right - 1;
	if (y2 >= m_ClipRect.bottom)
		y2 = m_ClipRect.bottom - 1;
...
HRESULT SoESurface::DrawTransAlphaMask(SoESurface* dest, LONG lDestX, LONG lDestY, RECT* srcRect)
{
	int register i, j;
	int height, width;
	BYTE* lpSprite;
...
BOOL SoESurface::ValidateBlt(SoESurface* lpSoES, LONG *lDestX, LONG *lDestY, RECT *srcRect)
{
	RECT destRect;
	RECT tmpRect;
	...
	// Test if destRect lies somewhere on lpSoES surface's clipRect
	if (!(lpSoES->ClipRect(&destRect)))
		return FALSE;
...
BOOL SoESurface::ClipRect(RECT *Rect)
{
...
void SoESurface::SetClipRect(RECT *clipRect)
{
...
HRESULT SoESurface::DrawBlk(SoESurface* lpSoES, LONG lDestX, LONG lDestY, RECT* srcRect)
{
	HRESULT rval;
    RECT tRect;
	...
	if (ValidateBlt(lpSoES, &lDestX, &lDestY, &tRect) == FALSE)
		return 0;
	
	rval = lpSoES->m_lpDDS->BltFast(lDestX, lDestY, m_lpDDS, &tRect, DDBLTFAST_WAIT);
...
HRESULT SoESurface::DrawBlkSW(SoESurface* lpSoES, LONG lDestX, LONG lDestY, RECT* srcRect)
{
	HRESULT rval = 0;
    LONG y;
    LONG w, h;
...
HRESULT SoESurface::DrawTrans(SoESurface* lpSoES, LONG lDestX, LONG lDestY, RECT* srcRect)
{
	HRESULT rval;
    RECT tRect;
	...
	rval = lpSoES->m_lpDDS->BltFast(lDestX, lDestY, m_lpDDS, &tRect, DDBLTFAST_WAIT | DDBLTFAST_SRCCOLORKEY);
...
HRESULT SoESurface::DrawTransSW(SoESurface* lpSoES, LONG lDestX, LONG lDestY, RECT* srcRect)
{
	HRESULT rval = 0;
	LONG i, j;
    LONG width, height;
...
HRESULT SoESurface::DrawBlkHFlip(SoESurface* lpSoES, LONG lDestX, LONG lDestY, RECT* srcRect)
{
	HRESULT rval;
	DDBLTFX ddBltFx;
	RECT destRect;
	...
	rval = lpSoES->m_lpDDS->Blt(&destRect, m_lpDDS, &tRect, DDBLT_DDFX | DDBLT_WAIT, &ddBltFx);
	...
	else
	{
		...
		for (int j = 0; j < height; j++)
		{
			for (int i = 0; i < width; i++)
			{
				...
				lpSoES->PutPixel((destRect.left + width) - i, destRect.top + j, Pixel);
			}
		}
...
HRESULT SoESurface::DrawTransHFlip(SoESurface* lpSoES, LONG lDestX, LONG lDestY, RECT* srcRect)
{
	HRESULT rval;
	DDBLTFX ddBltFx;
	RECT destRect;
	...
	rval = lpSoES->m_lpDDS->Blt(&destRect, m_lpDDS, &tRect, DDBLT_DDFX | DDBLT_WAIT | DDBLT_KEYSRC, &ddBltFx);
...
HRESULT SoESurface::DrawBlkVFlip(SoESurface* lpSoES, LONG lDestX, LONG lDestY, RECT* srcRect)
{
	HRESULT rval;
	DDBLTFX ddBltFx;
	RECT destRect;
	...
	else
	{
		...
		for (int j = 0; j < height; j++)
		{
			...
			{
				Pixel = GetPixel(tRect.left + i, tRect.top + j);
				lpSoES->PutPixel(destRect.left + i, (destRect.top + height) - j, Pixel);
			}
		}
...
HRESULT SoESurface::DrawTransVFlip(SoESurface* lpSoES, LONG lDestX, LONG lDestY, RECT* srcRect)
{
	HRESULT rval;
	DDBLTFX ddBltFx;
	RECT destRect;
...
HRESULT SoESurface::DrawBlkAlphaFast(SoESurface* dest, LONG lDestX, LONG lDestY, RECT* srcRect)
{
    int register i, j;
	int height, width;
    BYTE* lpSprite;
...
HRESULT SoESurface::DrawTransAlphaFast(SoESurface* dest, LONG lDestX, LONG lDestY, RECT* srcRect)
{
    int register i, j;
	int height, width;
    BYTE* lpSprite;
...
HRESULT SoESurface::DrawBlkAlpha(SoESurface* dest, LONG lDestX, LONG lDestY, RECT* srcRect, WORD ALPHA)
{
	int register i, j;
	int height, width;
	BYTE* lpSprite;
...
HRESULT SoESurface::DrawTransAlpha(SoESurface* dest, LONG lDestX, LONG lDestY, RECT* srcRect, WORD ALPHA)
{
	int register i, j;
	int height, width;
	BYTE* lpSprite;
...
HRESULT SoESurface::DrawBlkShadow(SoESurface* dest, LONG lDestX, LONG lDestY, RECT* srcRect, WORD SHADOW)
{
	int register i, j;
	int height, width;
	BYTE* lpSprite;
...
HRESULT SoESurface::DrawTransShadow(SoESurface* dest, LONG lDestX, LONG lDestY, RECT* srcRect, WORD SHADOW)
{
	int register i, j;
	int height, width;
	BYTE* lpSprite;
...
HRESULT SoESurface::DrawBlkShadowFast(SoESurface* dest, LONG lDestX, LONG lDestY, RECT* srcRect)
{
    int register i, j;
	int height, width;
    BYTE* lpSprite;
...
HRESULT SoESurface::DrawTransShadowFast(SoESurface* dest, LONG lDestX, LONG lDestY, RECT* srcRect)
{
    int register i, j;
	int height, width;
    BYTE* lpSprite;
...
HRESULT SoESurface::DrawBlkScaled(SoESurface* lpSoES, LONG lDestX, LONG lDestY, RECT* srcRect, float SF)

{
	...
	rval = lpSoES->m_lpDDS->Blt(&destRect, m_lpDDS, &tRect, DDBLT_WAIT, NULL);
	if (rval == DDERR_SURFACELOST) 
		Restore();
...
HRESULT SoESurface::DrawTransScaled(SoESurface* lpSoES, LONG lDestX, LONG lDestY, RECT* srcRect, float SF)
{
	HRESULT rval;
	RECT destRect;
	RECT tmpRect;
	...
	// Use the hardware to draw the image
	rval = lpSoES->m_lpDDS->Blt(&destRect, m_lpDDS, &tRect, DDBLT_WAIT | DDBLT_KEYSRC, NULL);
	if (rval == DDERR_SURFACELOST) 
		Restore();
...
HRESULT SoESurface::DrawBlkStretched(SoESurface* lpSoES, LONG lDestX, LONG lDestY, RECT* srcRect, LONG width, LONG height)
{
	HRESULT rval;
	RECT destRect;
	RECT tmpRect;
	...
	// From srcRect build the destRect
	destRect.top    = lDestY;
	//(LONG)( ((float)(lDestY) + (h/2.0)) - ( (float)height/2.0) );
	destRect.left   = lDestX;
	//(LONG)( ((float)(lDestX) + (w/2.0)) - ( (float)width/2.0) );
...
HRESULT SoESurface::DrawTransStretched(SoESurface* lpSoES, LONG lDestX, LONG lDestY, RECT* srcRect, LONG width, LONG height)
{
	HRESULT rval;
	RECT destRect;
	RECT tmpRect;
...
HRESULT SoESurface::Fade(SoESurface* dest, LONG lDestX, LONG lDestY, RECT* srcRect, WORD ALPHA)
{
	int register i, j;
...
char* SoESurface::GetFileName(char* name)
{ 
	if (m_FileName != NULL)
...
char* SoESurface::GetFontName(char* name)
{ 
	if (m_FontName != NULL)
...
void SoESurface::GetPixelFormat(RGBFORMAT* format)
{
...
HRESULT SoESurface::SetColorKey(DWORD col)
{
	DDCOLORKEY ddck;
    HRESULT rval;
...
HRESULT SoESurface::SetColorKey(void)
{
    DDCOLORKEY		ddck;
...
HRESULT SoESurface::Restore(void)
{
	HRESULT rval;
...
HRESULT SoESurface::UnLock(void)
{
	HRESULT rval;
...
HRESULT SoESurface::ReleaseDC(void)
{
    HRESULT rval;
...
HRESULT SoESurface::ChangeFont(const char* FontName, int Width, int Height, int Attributes)
{
    HRESULT rval;
...
HRESULT SoESurface::SetFont(void)
{
    HGDIOBJ rval;
    HDC dc;
...
HRESULT SoESurface::TextXY(int x, int y, COLORREF col, LPCTSTR pString)
{
    HRESULT rval;
    HDC dc;
...
HRESULT SoESurface::DrawText(LPCSTR pString, COLORREF col, LPRECT pRect)
{
    HRESULT rval;
    HDC dc;
...
HRESULT SoESurface::PutPixel8(int X, int Y, DWORD Col)
{
    BYTE* Bitmap;
...
HRESULT SoESurface::PutPixel16(int X, int Y, DWORD Col)
{
    BYTE* Bitmap;
...
HRESULT SoESurface::PutPixel24(int X, int Y, DWORD Col)
{
    BYTE* Bitmap;
...
HRESULT SoESurface::PutPixel32(int X, int Y, DWORD Col)
{
    BYTE* Bitmap;
...
HRESULT SoESurface::PutAAPixel8(int X, int Y, DWORD Col, WORD ALPHA)
{
    BYTE* Bitmap;
...
HRESULT SoESurface::PutAAPixel15(int X, int Y, DWORD Col, WORD ALPHA)
{
	DWORD db, dg, dr;
	DWORD sb, sg, sr;
	DWORD sTemp, dTemp;
...
HRESULT SoESurface::PutAAPixel16(int X, int Y, DWORD Col, WORD ALPHA)
{
	DWORD db, dg, dr;
	DWORD sb, sg, sr;
	DWORD sTemp, dTemp;
...
HRESULT SoESurface::PutAAPixel24(int X, int Y, DWORD Col, WORD ALPHA)
{
	DWORD db, dg, dr;
	DWORD sb, sg, sr;
	DWORD sTemp, dTemp;
...
HRESULT SoESurface::PutAAPixel32(int X, int Y, DWORD Col, WORD ALPHA)
{
	DWORD db, dg, dr;
	DWORD sb, sg, sr;
	DWORD sTemp, dTemp;
...
DWORD SoESurface::GetPixel8(int X, int Y)
{
	DWORD Pixel = NULL;
	BYTE* Bitmap;
...
DWORD SoESurface::GetPixel16(int X, int Y)
{
	DWORD Pixel = NULL;
	BYTE* Bitmap;
...
DWORD SoESurface::GetPixel24(int X, int Y)
{
	DWORD Pixel = NULL;
	BYTE* Bitmap;
...
DWORD SoESurface::GetPixel32(int X, int Y)
{
	DWORD Pixel = NULL;
	BYTE* Bitmap;
...
HRESULT SoESurface::Rect(int X1, int Y1, int X2, int Y2, DWORD Col)
{
    HRESULT rval;
...
HRESULT SoESurface::RoundedRect(int X1, int Y1, int X2, int Y2, int Radius, DWORD Col)
{
...
HRESULT SoESurface::FillRect(int X1, int Y1, int X2, int Y2, DWORD Col)
{
	DDBLTFX ddbltfx;
...
HRESULT SoESurface::Line(int X1, int Y1, int X2, int Y2, DWORD Col)
{
...
HRESULT SoESurface::AALine(int X1, int Y1, int X2, int Y2, DWORD Col)
{
...
HRESULT SoESurface::VLine8(int Y1, int Y2, int X, DWORD Col)
{
...
HRESULT SoESurface::VLine16(int Y1, int Y2, int X, DWORD Col)
{
    int Length, top, bottom;
    BYTE* Pixel;
...
HRESULT SoESurface::VLine24(int Y1, int Y2, int X, DWORD Col)
{
    int Length, top, bottom;
	DWORD dwCol;
    BYTE* Pixel;
...
HRESULT SoESurface::VLine32(int Y1, int Y2, int X, DWORD Col)
{
    int Length, top, bottom;
	DWORD dwCol;
    BYTE* Pixel;
...
HRESULT SoESurface::HLine8(int X1, int X2, int Y, DWORD Col)
{
...
HRESULT SoESurface::HLine16(int X1, int X2, int Y, DWORD Col)
{
    int Length, left, right;
	int i;
    DWORD dwCol;
...
HRESULT SoESurface::HLine24(int X1, int X2, int Y, DWORD Col)
{
    int Length, left, right;
    DWORD dwCol;
    BYTE* Pixel;
...
HRESULT SoESurface::HLine32(int X1, int X2, int Y, DWORD Col)
{
    int Length, left, right;
    DWORD dwCol;
    BYTE* Pixel;
...
HRESULT SoESurface::Circle(int X, int Y, int Radius, DWORD Col)
{
...
HRESULT SoESurface::FillCircle(int X, int Y, int Radius, DWORD Col)
{
    int Y1, dx;
    int top, bot;
	double R;
...
HRESULT SoESurface::Fill(DWORD FillColor)
{
	DDBLTFX ddBltFx;
    HRESULT rval;
...
void SoESurface::GetSurfaceDescriptor(SOE_LPDDSURFACEDESC lpddsd)
{
    *lpddsd = m_DDSD;
}

HRESULT SoESurface::ReLoadBitmap(const char* fileName)
{
    ...
    // Load the file to a DX surface
    lpDDS = image->GetImage(GetScreen()->GetDD(), fileName, SOEMEM_SYSTEMONLY, SOEIMAGE_UNKNOWN);
...
HRESULT SoESurface::SaveAsBMP(const char * szFilename)
{
	HANDLE file_out;
    HRESULT rval;
    DWORD numwrite;
...
void SoESurface::Scanrightside(int x1, int x2, int ytop, int lineheight, char side, int TexWidth, int TexHeight, RECT* dClip)
{
...
void SoESurface::Scanleftside(int x1, int x2, int ytop, int lineheight, char side, int TexWidth, int TexHeight, RECT* dClip)
{
	int x, px, py, xadd, pxadd, pyadd, y;
...
void SoESurface::TextureMap(SoESurface *dest, int x1, int y1, int x2, int y2, int x3, int y3, int x4, int y4, RECT* area, BOOL bTrans)
{
    BYTE* lpSrc;
	BYTE* lpDest;
	unsigned long dPitch, sPitch;
	...
	if (miny >= maxy)
		return;
	if (maxy < dClipRect->top)
		return;
	if (miny >= dClipRect->bottom)
		return;
	if (miny < dClipRect->top)
		miny = dClipRect->top;
	if (maxy > dClipRect->bottom)
		maxy = dClipRect->bottom;
	if (maxy - miny < 1)
		return;
...
HRESULT SoESurface::DrawBlkRotoZoom(SoESurface* dest, int midX, int midY, RECT* area, double angle, double scale)
{
	double SinA, CosA;
    BOOL bTrans = FALSE;
...
HRESULT SoESurface::DrawTransRotoZoom(SoESurface* dest, int midX, int midY, RECT* area, double angle, double scale)
{
	double SinA, CosA;
    BOOL bTrans = TRUE;
