...
BOOL SoEImageTGA::Validate(const CHAR *head, const int headsize, const CHAR *foot, const int footsize)
{
...
SOE_LPDIRECTDRAWSURFACE SoEImageTGA::GetImage(SOE_LPDIRECTDRAW lpDD, LONG lSize, CHAR* lpCache, BYTE memType)
{
...
void SoEImageTGA::t16_t24(BYTE *dest, const BYTE *src, int n)
{
	while (n--)
	{
		WORD l = *src++;
...
void SoEImageTGA::t32_t24(BYTE *dest, const BYTE *src, int n)
{
	while (n--)
	{
		*dest++ = *src++;
